...
fun strategy_from_mimicking_of_C :: \<open>('s \<Rightarrow> ('s set) \<Rightarrow> bool) \<Rightarrow> ('s, 'a) c_set_game_node strategy\<close>
  where
...
proof (induct arbitrary: n p Q rule: plays_for_0strategy.induct[OF assms(3)])
  ...
  thus  \<open>mimicking (set_lifted C) p Q\<close> using assms R_is_in_mimicking_of_R set_lifted_def by simp
next
  case p0moved: (2 n0 play) 
  hence \<open>(n = strategy_from_mimicking_of_C (mimicking (set_lifted C)) (n0 # play)) \<or> (n \<in> set (n0#play))\<close> by auto
  thus ?case
  proof(rule disjE)
    assume \<open>n \<in> set (n0#play)\<close>
    thus ?thesis using p0moved.prems p0moved.hyps(1,2) by blast 
  next
    assume strat: \<open>n = strategy_from_mimicking_of_C (mimicking (set_lifted C)) (n0 # play)\<close>
    ...
    proof (rule disjE)
      ...
      hence \<open>hd play \<in> set (n0 # play)\<close> 
        ...
        by (simp add: assms(3))
      hence \<open>mimicking (set_lifted C) ppred Qpred\<close> using p0moved.hyps(2) ppred_def by blast
      hence \<open>mimicking (set_lifted C) p (dsuccs a Qpred)\<close> 
       using \<open>ppred =\<rhd>a p\<close> assms(1,2) mimicking_of_C_guarantees_action_succ \<open>a \<noteq> \<tau>\<close> by auto
    ...
    next
      ...
      then obtain q Ppred where n0_def: \<open>n0 = DefenderSwapNode q Ppred\<close> and Q_def: \<open>Q = {q}\<close> 
        by auto
      hence \<open>strategy_from_mimicking_of_C (mimicking (set_lifted C)) (n0#play)
         = AttackerNode (SOME p1. (\<exists>p. p \<in> Ppred \<and> p \<Rightarrow>^\<tau> p1) \<and> (mimicking (set_lifted C)) p1 {q}) {q}\<close> 
        using strategy_from_mimicking_of_C.simps(1) by auto
      hence p_def: \<open>p = (SOME p1. (\<exists>p. p \<in> Ppred \<and> p \<Rightarrow>^\<tau> p1) \<and> (mimicking (set_lifted C)) p1 {q})\<close> 
      ...
      have \<open>hd play \<in> set (n0 # play)\<close> 
        using second_elem_in_play_set strategy0_plays_subset[OF p0moved.hyps(1)] assms(3) n0_def
        by simp 
      hence \<open>mimicking (set_lifted C) qpred Ppred\<close> using p0moved.hyps(2) qpred_def by blast
...
proof (safe)
  fix play
  assume A1: \<open>play \<in> (plays_for_0strategy (strategy_from_mimicking_of_C (mimicking (set_lifted C))) (AttackerNode p0 {q0}))\<close>
  ...
  proof - 
    ...
    next
      ...
      hence \<open>\<not>c_set_game_defender_node (strategy_from_mimicking_of_C (mimicking (set_lifted C)) (n0#play))\<close>
        using p0moved.hyps(4) 
          c_set_game_moves.elims(2)[of \<open>n0\<close> \<open>strategy_from_mimicking_of_C (mimicking (set_lifted C)) (n0#play)\<close>]
        by force 
      hence \<open>\<not>c_set_game_defender_node (hd (strategy_from_mimicking_of_C (mimicking (set_lifted C)) (n0 # play) # n0 # play))\<close> 
        by simp
...
proof (safe)
  fix n0 play
  assume A: 
    \<open>n0 # play \<in> plays_for_0strategy (strategy_from_mimicking_of_C (mimicking (set_lifted C))) (AttackerNode p0 {q0})\<close>
    \<open>c_set_game_defender_node n0\<close>
  hence \<open>(\<exists>a p' Q. n0 = DefenderSimNode a p' Q) \<or> (\<exists>p' Q. n0 = DefenderSwapNode p' Q)\<close>
    by (meson c_set_game_defender_node.elims(2))
  thus \<open>c_set_game_moves n0 (strategy_from_mimicking_of_C (mimicking (set_lifted C)) (n0 # play))\<close>
  proof(rule disjE)
    ...
    hence  \<open>mimicking (set_lifted C) p' (dsuccs a Q)\<close>
      using mimicking_of_C_guarantees_action_succ \<open>\<not> tau a\<close> \<open>p =\<rhd>a p'\<close> assms(1) tau_tau by blast 
    hence Q1_ex: \<open>\<exists>Q'. Q' = dsuccs a Q \<and> mimicking (set_lifted C) p' Q'\<close> by auto
    from n0_def have st: \<open>strategy_from_mimicking_of_C (mimicking (set_lifted C)) (n0#play) = 
    (AttackerNode p' (SOME Q1 . Q1 = dsuccs a Q \<and> (mimicking (set_lifted C)) p' Q1))\<close> by auto
    then obtain Q1 where 
      \<open>AttackerNode p' (SOME Q1 . Q1 = dsuccs a Q \<and> (mimicking (set_lifted C)) p' Q1) = AttackerNode p' Q1\<close>
      by blast
    hence Q1_def: \<open>Q1 = (SOME Q1 . Q1 = dsuccs a Q \<and> (mimicking (set_lifted C)) p' Q1)\<close>
      by auto
    have next_is_atk: \<open>strategy_from_mimicking_of_C (mimicking (set_lifted C)) (n0#play) = (AttackerNode p' Q1)\<close> 
      using st Q1_def by auto
    with someI_ex[OF Q1_ex] Q1_def have mov_cond: \<open>Q1 = dsuccs a Q \<and> mimicking (set_lifted C) p' Q1\<close>
      by blast
  ...
  next 
    ...
    hence q1_ex: \<open>\<exists>q1. (\<exists>q.(q \<in> Q \<and> q  \<Rightarrow>^\<tau> q1)) \<and> mimicking (set_lifted C) q1 {p'}\<close> using weak_tau_succs_def by auto
    hence strat: \<open>strategy_from_mimicking_of_C (mimicking (set_lifted C)) (n0#play) = 
    AttackerNode (SOME q1 . (\<exists>q. (q \<in> Q \<and> q  \<Rightarrow>^\<tau> q1)) \<and> (mimicking (set_lifted C)) q1 {p'}) {p'}\<close>
      using n0_def by auto
    then obtain q1 where 
      \<open>AttackerNode (SOME q1 . (\<exists>q. (q \<in> Q \<and> q  \<Rightarrow>^\<tau> q1)) \<and> (mimicking (set_lifted C)) q1 {p'}) {p'}
      = AttackerNode q1 {p'}\<close> by blast
    hence q1_def: \<open>q1 = (SOME q1 . (\<exists>q. (q \<in> Q \<and> q  \<Rightarrow>^\<tau> q1)) \<and> (mimicking (set_lifted C)) q1 {p'})\<close>
      by auto
    with someI_ex[OF q1_ex] have X: \<open>\<exists>q. (q \<in> Q \<and> q  \<Rightarrow>^\<tau> q1) \<and> mimicking (set_lifted C) q1 {p'}\<close> 
      by blast
